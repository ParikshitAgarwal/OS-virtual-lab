...
function createTable(tablename,frames,ref){
    ...
    let table = '<tr><td id="'+tablename+'00" style="font-weight:bolder;width:130px;height:41px">Reference</td>';
    for (let i = 0; i < ref.length; i++) {
        table += '<td style="font-weight:bolder;width:40px;height:41px" id="'+tablename+'0'+(i+1) + '">' + ref[i] + "</td>";
    }
    table += "</tr>";
    for (let i = 0; i < frames; i++) {
        table += '<tr><td style="font-weight:bolder;width:100px;height:41px"id="' +tablename+ (i + 1) + '0">Frame ' + (i + 1) + "</td>";
    ...
    frames++;
    table += '<tr><td style="font-weight:bolder;width:100px;height:41px"id="' +tablename+ frames + '0">Status</td>';
    for (var j = 1; j <= ref.length; j++) {
        table += '<td style="width:60px;" id="' +tablename+ frames + j + '"></td>';
    }
    table += "</tr>";
    console.log(table);
